...
### 原架构（依赖 PyPI）
```
插件后端 → subprocess → python -m uiautodev → PyPI 安装的包
```

### 新架构（本地源代码）
```
插件后端 → subprocess → uiautodev/__main__.py → 本地源代码
```

### 关键改动

1. **源代码集成**
   - 将 `uiautodev` 源代码完整复制到插件目录
...
1. **查看日志**：插件后端会将 uiautodev 的 stdout/stderr 转发到前端
